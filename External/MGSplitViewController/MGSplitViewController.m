		...
		if (controller && [controller isKindOfClass:[UIViewController class]])  {
			...
			if (theView) {
				...
				if (!theView.superview) {
					if (_notifyChildViewControllersOfViewLifecycleDelegateMethodCalls) [controller viewWillAppear:NO];
					[self.view addSubview:theView];
					if (_notifyChildViewControllersOfViewLifecycleDelegateMethodCalls) [controller viewDidAppear:NO];
				}
			}
		}
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (_notifyChildViewControllersOfViewLifecycleDelegateMethodCalls) {
...
- (void)reconfigureForMasterInPopover:(BOOL)inPopover
{
	...
	if (inPopover && !_hiddenPopoverController && !_barButtonItem) {
		...
		if (_notifyChildViewControllersOfViewLifecycleDelegateMethodCalls) [self.masterViewController viewWillDisappear:NO];
		_hiddenPopoverController = [[UIPopoverController alloc] initWithContentViewController:self.masterViewController];
		if (_notifyChildViewControllersOfViewLifecycleDelegateMethodCalls) [self.masterViewController viewDidDisappear:NO];
...
@synthesize notifyChildViewControllersOfViewLifecycleDelegateMethodCalls = _notifyChildViewControllersOfViewLifecycleDelegateMethodCalls; // See issue MOBBL-150
