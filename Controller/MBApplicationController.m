...
- (id) init
{
	...
	if (self != nil) {
        _instance = self;
		// Added for optimization: Make sure the MBDeviceType is created at startup. The createInstance method instantiate variables that only need to be gathered once in the application lifecycle 
		[MBDeviceType createInstance]; 
	}
	return self;
}
...
- (void) handleException:(NSException*) exception outcome:(MBOutcome*) outcome {
    ...
    if([outcomeDefinitions count] != 0) {
		...
		MBOutcome *genericExceptionHandler = [[MBOutcome alloc] initWithOutcomeName:@"exception" document: exceptionDocument];
